...
def build_proxy_list(proxy: str, proxy_ports: str | None) -> tuple[list[str], list[int]]:
    ...
    return [f"{scheme}://{host}:{port}" for port in ports], ports
...
def main() -> None:
    ...
    proxies, proxy_ports = build_proxy_list(args.proxy, args.proxy_ports)
    port_index_map: dict[int, int] = {port: idx for idx, port in enumerate(proxy_ports)}
            ...
            try:
                if args.pcap_dir is not None:
                    ...
                    if args.pcap_follow_proxy_ports and entry_port is not None:
                        offset = port_index_map.get(entry_port)
                        if offset is None:
                            offset = entry_port - args.pcap_entry_port_base
                        exit_port = args.pcap_exit_port_base + offset
                        middle_offset = offset * len(base_middle_ports)
                        middle_ports = [
                            port + middle_offset for port in base_middle_ports
                        ]
                    if not middle_ports:
                        middle_ports = list(base_middle_ports)
                    ...
                    if capture_proc.poll() is not None:
                        stdout, stderr = capture_proc.communicate()
                        detail = stderr.strip() if stderr else stdout.strip()
                        message = "抓包进程启动后已退出，可能缺少权限或 tcpdump/tshark 不可用。"
                        if detail:
                            message = f"{message} 错误信息：{detail}"
