...
def extract_features(
    ...
    *,
    ...
    if reference_root is not None:
        reference = build_reference_distribution(reference_root, group_level, bins, suffix)
    ...
    total_pcaps = 0
    bad_pcaps: list[str] = []
    for pcap, label, group in collect_pcaps(root, group_level, suffix):
        total_pcaps += 1
        try:
            packets = load_pcap_packets(pcap)
            ...
            continue
        if len(packets) < min_pkts:
            skipped += 1
            continue
        features = build_feature_vector(packets, max_pkts, max_bursts, reference, bins)
        if feature_len is None:
            feature_len = len(features)
        rows.append(
            {"label": label, "group": group, **{f"f{i}": v for i, v in enumerate(features)}}
        )
    if feature_len is None:
        raise RuntimeError(
    ...
    return rows, feature_len, skipped, total_pcaps, bad_pcaps


def write_features_csv(output_csv: Path, rows: list[dict[str, object]]) -> None:
...
def write_features_jsonl(output_path: Path, rows: list[dict[str, object]]) -> None:
...
def write_features_npz(output_path: Path, rows: list[dict[str, object]]) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    feature_names = [key for key in rows[0].keys() if key.startswith("f")]
    feature_names = sorted(feature_names, key=lambda name: int(name[1:]))
    ...
    groups = np.array([row["group"] for row in rows], dtype=str)
    np.savez_compressed(
        output_path,
        features=features,
        labels=labels,
...
def write_features(
    ...
    *,
    ...
    if output_format == "csv":
        ...
        write_features_jsonl(output_path, rows)
    elif output_format == "npz":
        write_features_npz(output_path, rows)
    else:
...
def main() -> None:
    args = parse_args()
    rows, _feature_len, skipped, _total_pcaps, bad_pcaps = extract_features(
        args.pcap_root,
